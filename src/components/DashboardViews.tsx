...
export function ConnectionsView({ models, projectPath, ollamaHealth, ollamaHealthChecking, onCheckOllama, onOpenSetup }: { models: ModelInfo[]; projectPath?: string; ollamaHealth?: OllamaHealth; ollamaHealthChecking: boolean; onCheckOllama: () => void; onOpenSetup: (kind: "provider" | "mcp") => void }) {
  ...
  return (
    <div className="page-scroll standard-page connections-page">
      ...
      <section className="connections-section">
        <SectionHeader title={t("connections.mcpTitle")} subtitle={t("connections.mcpSubtitle")} action={<><Button variant="ghost" onClick={() => void openOfficialSetup("/mcp list", t("connections.mcpTitle"))}><Terminal size={15} />{t("connections.openMcp")}</Button><Button variant="secondary" onClick={() => setMcpEditor(emptyMcpDraft())}><Plus size={15} />{t("connections.addServer")}</Button></>} />
        <div className="mcp-grid">
          ...
          {customMcp.map((server) => { const legacyProject = server.scope === "project"; const deletable = Boolean(server.url); const editable = deletable && !server.hasCustomHeaders && !legacyProject; const status = legacyProject ? t("connections.projectScopeIgnored") : !deletable ? t("connections.kernelUnsupported") : !server.enabled ? t("connections.disabled") : server.hasCustomHeaders ? t("connections.customHeaders") : authKindLabel(server.authKind, t); const editTitle = editable ? t("common.edit") : legacyProject ? t("connections.projectScopeMigration") : server.hasCustomHeaders ? t("connections.customHeadersReadonly") : t("connections.stdioEditDisabled"); return <article key={`${server.scope}:${server.name}`} className="mcp-card"><span className="mcp-logo custom-server-logo"><Server size={18} /></span><div><strong>{server.name}</strong><p>{server.url ?? t("connections.stdioReadonly")}</p><small><span className={`status-dot ${server.enabled && !legacyProject ? "is-online" : ""}`} />{status} · {legacyProject ? t("connections.projectScope") : t("connections.globalScope")}</small></div><div className="mcp-card-actions"><Button variant="ghost" disabled={!editable} title={editTitle} onClick={() => setMcpEditor(mcpDraftFromServer(server))}>{t("common.edit")}</Button><button type="button" className="inline-danger" disabled={!deletable} aria-label={t("projects.deleteLabel", { name: server.name })} title={deletable ? t("common.delete") : t("connections.stdioKept")} onClick={() => setMcpToDelete(server)}><Trash2 size={14} /></button></div></article>; })}
          <button type="button" className="mcp-card mcp-custom" onClick={() => setMcpEditor(emptyMcpDraft())}><span><Plus size={20} /></span><div><strong>{t("connections.customServer")}</strong><p>{t("connections.customServerText")}</p></div><ChevronRight size={16} /></button>
        </div>
      ...
      {mcpEditor ? <McpEditorModal draft={mcpEditor} onClose={() => setMcpEditor(undefined)} onSave={saveMcp} /> : null}
...
function emptyMcpDraft(): McpEditorDraft {
  return { name: "", url: "", enabled: true, scope: "global", authKind: "none", bearerTokenEnvVar: "", existing: false };
}
...
function McpEditorModal({ draft: initialDraft, onClose, onSave }: { draft: McpEditorDraft; onClose: () => void; onSave: (draft: McpEditorDraft) => Promise<void> }) {
  const { t } = useI18n();
